...
public class StampsController {
    ...
    @ApiResponse(responseCode = "409", description = "생성 실패(중복)", content = @Content(schema = @Schema(implementation = Integer.class)))
    @PostMapping("/v3/save")
    ...
    @ApiResponse(responseCode = "204", description = "조회 실패(존재x)", content = @Content(schema = @Schema(implementation = Integer.class)))
    ...
    public ResponseEntity<?> findByTouristAttractionsIdDesc3(@PathVariable String touristAttractionsId) {
        ...
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("리소스가 존재하지 않습니다");
    }
    ...
    @ApiResponse(responseCode = "204", description = "조회 실패(사용자 존재x)", content = @Content(schema = @Schema(implementation = Integer.class)))
